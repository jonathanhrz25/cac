...
$hojaActiva->setCellValue('B5', 'Numero de cliente');
$hojaActiva->setCellValue('C5', 'Nombre y puesto o relación con el titular');
$hojaActiva->setCellValue('D5', '1. ¿Sigue en apertura su negocio?');
$hojaActiva->setCellValue('E5', '2. ¿Cuál es el motivo de su inactividad de compra con nosotros?');
$hojaActiva->setCellValue('F5', 'Otro motivo');
$hojaActiva->setCellValue('G5', '3. De acuerdo a la respuesta anterior, colocar la situación presentada con Serva en caso de que aplique.');
$hojaActiva->setCellValue('H5', '4. ¿Le interesaría retomar la relación comercial con Serva?');
...
$hojaActiva->setCellValue('B'.$fila, $rows['num_cliente']);
$hojaActiva->setCellValue('C'.$fila, $rows['nombre']);
$hojaActiva->setCellValue('D'.$fila, $rows['negocio']);
$hojaActiva->setCellValue('E'.$fila, $rows['inactividad']);
$hojaActiva->setCellValue('F'.$fila, $rows['otroTexto']);
$hojaActiva->setCellValue('G'.$fila, $rows['situacion']);
$hojaActiva->setCellValue('H'.$fila, $rows['relacion_com']);
$fila++;
}
...
$excel->setActiveSheetIndex(0)->setCellValue('C1', 'Reporte Encuestas CAC - Bloque 1');
...
$style = $hojaActiva->getStyle('H5:H100');
$style->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);
$style->getAlignment()->setVertical(\PhpOffice\PhpSpreadsheet\Style\Alignment::VERTICAL_CENTER);
...
header('Content-Disposition: attachment;filename="Reporte_Cac.xlsx"');
