...
<body>
    <header>
        <nav class="navbar navbar-expand-lg navbar-dark bg-dark fixed-top" style="background-color: #081856!important;">
            <a class="navbar-brand" href="../ejecutivasCac/bloque1.php">
            ...
            function drawChart() {
                var data = google.visualization.arrayToDataTable([
                    ...
                    foreach ($categorias as $categoria) {
                        $sqlCategoria = "SELECT COUNT(*) as total FROM form_clientes WHERE negocio = '$categoria'";
        ...
        <script>
            ...
            function drawChart() {
                var data = google.visualization.arrayToDataTable([
                    ['Calidad de Servicio', 'Cantidad'],
                    <?php
                    $categorias = ["Ventas bajas", "Tiempos de entrega", "Surtido", "Devoluciones y garantias", "Falta de visita del vendedor", "Credito y Cobranza", "Precios", "Mala experiencia de compra", "Disponibilidad de material", "Mala atencion de TLK", "Mala calidad del producto", "Otro"];
                    foreach ($categorias as $categoria) {
                        $sqlCategoria = "SELECT COUNT(*) as total FROM form_clientes WHERE inactividad = '$categoria'";
                        $resultCategoria = mysqli_query($conn, $sqlCategoria);
                        $rowCategoria = mysqli_fetch_assoc($resultCategoria);
                        echo "['$categoria', " . $rowCategoria['total'] . "],";
                    }
        ...
        <script>
            ...
            function drawChart() {
                var data = google.visualization.arrayToDataTable([
                    ...
                    $categorias = ["Si", "No"];
                    foreach ($categorias as $categoria) {
                        $sqlCategoria = "SELECT COUNT(*) as total FROM form_clientes WHERE relacion_com = '$categoria'";
                        $resultCategoria = mysqli_query($conn, $sqlCategoria);
                        $rowCategoria = mysqli_fetch_assoc($resultCategoria);
                        echo "['$categoria', " . $rowCategoria['total'] . "],";
                    }
